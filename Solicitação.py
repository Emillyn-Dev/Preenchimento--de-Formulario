...
# CONFIGURAÇÃO
# =========================
# Mudar pro caminho da pasta
os.chdir(r"x") #caminho da pasta do código

URL = "x" #url do site que vai rodar o código
PLANILHA = "RITM - INC.xlsx" #nome da planilha
SHEET = "RITM" #planilha que quero consultar pra abertura do chamado
...
try:
    ...
    print("❌ ERRO: Feche o arquivo 'RITM - INC.xlsx' no Excel antes de rodar o script!")
...
# DRIVER
# =========================
# Mudar pro caminho msedgedriver.exe
service = Service("x") #mudar aqui
    ...
    while time.monotonic() < fim:
        ...
        try:
            opcoes = driver.find_elements(
                By.CSS_SELECTOR, ".select2-results li.select2-result-selectable"
            )
            visiveis = [o for o in opcoes if o.is_displayed() and o.text.strip()]
        except Exception:
            visiveis = []
